...
final class FavoritesCoordinator {
    ...
    private var errorMessage = "收藏库尚未就绪。"
    ...
    func open(store: AppDataStore, available: Bool) {
        ...
        do {
            guard available else { throw LocalStorageError("当前存储不可用，请通过“数据与恢复”重试。") }
            repository = try FavoritesRepository(directoryURL: store.directoryURL)
            library.setRepository(repository)
        } catch { errorMessage = error is DataFormatError ? "收藏由更新版本创建，请使用更新版本打开。原文件已保留。" : error.localizedDescription; library.setRepository(nil,message:errorMessage) }
    }
    func suspend() {
        library.setRepository(nil,message:"正在迁移存储，新复制暂存于内存，完成后保存。")
    }
    ...
    func toggle(_ entry: HistoryEntry) {
        guard !library.isMutating else { onMessage?("收藏正在保存，请稍后操作。");return }
        guard let repository else { onMessage?(errorMessage);return }
        do {
            if let id = try repository.favoriteID(for:entry.payload) {
                try repository.remove(ids:[id]);onMessage?("已取消收藏，普通历史仍保留。")
            } else {
                try repository.add(entry, thumbnail: FavoriteThumbnail.make(for:entry.payload));onMessage?("已收藏到“未分类”，可在收藏库中整理。")
            }
