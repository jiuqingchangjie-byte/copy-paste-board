...
cp -R Resources/Localization/. "$APP/Contents/Resources/"
"$PROJECT_DIR/scripts/embed-sparkle.sh" "$APP" "$CODE_SIGN_IDENTITY"
/usr/libexec/PlistBuddy -c "Add ClipboardBoardBuildID string $(uuidgen)" "$APP/Contents/Info.plist"
/usr/libexec/PlistBuddy -c "Add ClipboardBoardSourceSHA256 string $(python3 "$PROJECT_DIR/scripts/build-inputs.py")" "$APP/Contents/Info.plist"
