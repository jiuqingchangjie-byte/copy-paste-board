...
final class ShortcutPreferencesController: NSWindowController, NSWindowDelegate {
    ...
    init() {
        let window = ShortcutPreferencesWindow(contentRect: NSRect(x: 0, y: 0, width: 440, height: 295),
        ...
        let title = NSTextField(labelWithString: "打开 / 关闭剪贴板历史")
        title.font = .systemFont(ofSize: 17, weight: .semibold)
        let hint = NSTextField(wrappingLabelWithString: "点击录制，再按组合键。至少包含 ⌘、⌥、⌃ 之一。\n按键按键盘位置保存，字母标识以美式键盘为准。")
        hint.font = .systemFont(ofSize: 12)
        hint.textColor = .secondaryLabelColor
        let record = NSButton(title: "录制快捷键", target: self, action: #selector(beginRecording))
        let reset = NSButton(title: "恢复默认 ⌥V", target: self, action: #selector(restoreDefault))
        let saveButton = NSButton(title: "保存", target: self, action: #selector(saveShortcut))
    ...
    func present(current: KeyboardShortcut, isRegistered: Bool, save: @escaping (KeyboardShortcut) throws -> Void) {
        ...
        currentLabel.stringValue = "当前设置：\(current.displayName)\(isRegistered ? " · 已生效" : " · 未注册，可重新设置")"
        showMessage("保存成功后新快捷键立即生效；失败时保留原快捷键。", error: false)
        window?.center()
    ...
    @objc func restoreDefault() {
        ...
        showMessage("已选择默认 ⌥V，点击保存后生效。", error: false)
    }
