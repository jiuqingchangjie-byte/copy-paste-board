...
"$PROJECT_DIR/scripts/embed-sparkle.sh" "$STAGED_APP" "$SIGNING_IDENTITY"
/usr/libexec/PlistBuddy -c "Add ClipboardBoardBuildID string $(uuidgen)" "$STAGED_APP/Contents/Info.plist"
/usr/libexec/PlistBuddy -c "Add ClipboardBoardSourceSHA256 string $(python3 "$PROJECT_DIR/scripts/build-inputs.py")" "$STAGED_APP/Contents/Info.plist"
