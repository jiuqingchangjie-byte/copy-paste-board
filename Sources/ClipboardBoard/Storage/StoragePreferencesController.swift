...
final class StoragePreferencesController: NSWindowController {
    ...
    private let change = NSButton(title:"更改存储位置…",target:nil,action:nil)
    ...
    init() {
        ...
        window.title = "存储位置";window.isReleasedWhenClosed = false
        let title = NSTextField(labelWithString:"历史、收藏与设置的存储位置")
        title.font = .systemFont(ofSize:17,weight:.semibold)
        pathLabel.isSelectable = true;pathLabel.maximumNumberOfLines = 4
        pathLabel.setAccessibilityLabel("当前数据存储目录")
        let detail = NSTextField(wrappingLabelWithString:"退出或电脑重启后仍从此处恢复。新位置会创建 ClipboardBoardData 文件夹；迁移校验成功后才切换，旧目录保留副本。")
        ...
        let open = NSButton(title:"打开当前目录",target:self,action:#selector(openDirectory))
        change.target = self;change.action = #selector(chooseDirectory)
        change.toolTip = "请选择稳定可写的目录；云盘目录可能由其客户端同步，应用不主动联网。"
    ...
    func present(directory:URL) {
        ...
        if !isMigrating { status.stringValue = "收藏独立保存，不受普通历史的 50 条上限影响。";status.textColor = .secondaryLabelColor }
    ...
    @objc private func chooseDirectory() {
        ...
        picker.prompt = "选择存放位置";picker.message = "在所选目录下创建 ClipboardBoardData；已有数据不会被覆盖。"
        picker.beginSheetModal(for:window) { [weak self] result in
            ...
            let alert = NSAlert();alert.messageText = "迁移到新存储位置？"
            alert.informativeText = target.path + "\n\n将复制历史、收藏与设置并校验。旧目录保留副本，切换后不会自动合并旧副本。"
            alert.addButton(withTitle:"迁移并使用");alert.addButton(withTitle:"取消")
            alert.beginSheetModal(for:window) { [weak self] choice in
                ...
                self.status.stringValue = "正在迁移并校验…新复制暂存于内存，完成后保存。"
                self.onMove?(target) { [weak self] result in
                    guard let self else { return }
                    self.isMigrating = false;self.change.isEnabled = true
                    switch result {
                    case .success(let url):self.directory = url;self.pathLabel.stringValue = url.path;self.status.stringValue = "已切换到新位置。旧目录仍保留副本。";self.status.textColor = .secondaryLabelColor
